 ...
 * available workers, determining when there are new tasks and assigning them to
 * available workers.
 * 
 * The flow without crashes is like this. The master reads the list of available
 * workers and watch for changes to the list of workers. It also reads the list
 * of tasks and watches for changes to the list of tasks. For each new task, it
 * assigns the task to a worker chosen at random.
 * 
 * Before exercising the role of master, this ZooKeeper client first needs to
 * elect a primary master. It does it by creating a /master znode. If it
 * succeeds, then it exercises the role of master. Otherwise, it watches the
 * /master znode, and if it goes away, it tries to elect a new primary master.
 * 
 * The states of this client are three: RUNNING, ELECTED, NOTELECTED. RUNNING
 * means that according to its view of the ZooKeeper state, there is no primary
 * master (no master has been able to acquire the /master lock). If some master
 * succeeds in creating the /master znode and this master learns it, then it
 * transitions to ELECTED if it is the primary and NOTELECTED otherwise.
 * 
 * Because workers may crash, this master also needs to be able to reassign
 * tasks. When it watches for changes in the list of workers, it also receives a
 * notification when a znode representing a worker is gone, so it is able to
 * reassign its tasks.
 * 
 * A primary may crash too. In the case a primary crashes, the next primary that
 * takes over the role needs to make sure that it assigns and reassigns tasks
 * that the previous primary hasn't had time to process.
 *
 */
public class Master implements Watcher, Closeable {
    ...
    /*
     * A master process can be either running for primary master, elected
     * primary master, or not elected, in which case it is a backup master.
     */
    enum MasterStates {
        RUNNING,
        ELECTED,
    ...
    /**
     * This method implements the process method of the Watcher interface. We
     * use it to deal with the different states of a session.
     * 
     * @param e
     *            new session event to be processed
     */
    ...
    /*
     * The story in this callback implementation is the following. We tried to
     * create the master lock znode. If it suceeds, then great, it takes
     * leadership. However, there are a couple of exceptional situations we need
     * to take care of.
     * 
     * First, we could get a connection loss event before getting an answer so
     * we are left wondering if the operation went through. To check, we try to
     * read the /master znode. If it is there, then we check if this master is
     * the primary. If not, we run for master again.
     * 
     * The second case is if we find that the node is already there. In this
     * case, we call exists to set a watch on the znode.
     */
     ...
     * with masteCreateCallback being the callback implementation. In the case
     * the create call succeeds, the client becomes the master. If it receives a
     * CONNECTIONLOSS event, then it needs to check if the znode has been
     * created. In the case the znode exists, it needs to check which server is
     * the master.
     */
     ...
     * 
     * @param path
     *            Path of assigned task
     * @param task
     *            Task name excluding the path prefix
     */
    ...
    void recreateTask(RecreateTaskCtx ctx) {
        zk.create("/tasks/" + ctx.task, ctx.data, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT,
            recreateTaskCallback, ctx);
    }
