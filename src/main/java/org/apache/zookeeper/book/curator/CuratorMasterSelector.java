...
public class CuratorMasterSelector implements Closeable, LeaderSelectorListener {
    ...
    public void takeLeadership(CuratorFramework client) throws Exception {
        ...
        (new RecoveredAssignments(client.getZookeeperClient().getZooKeeper()))
            .recover(new RecoveryCallback() {
            ...
            try {
                ...
                case CHILDREN:
                    if (event.getPath().contains("/assign")) {
                        LOG.info("Succesfully got a list of assignments: " + event.getChildren().size()
                                + " tasks");
                        /*
                ...
                case DELETE:
                    ...
                    if (event.getPath().contains("/tasks")) {
                        LOG.info("Result of delete operation: " + event.getResultCode() + ", "
                                + event.getPath());
                    }
    ...
    PathChildrenCacheListener tasksCacheListener = new PathChildrenCacheListener() {
        public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) {
            if (event.getType() == PathChildrenCacheEvent.Type.CHILD_ADDED) {
                try {
                    assignTask(event.getData().getPath().replaceFirst("/tasks/", ""), event.getData()
                        .getData());
                }
                catch (Exception e) {
    ...
    void assignTask(String task, byte[] data) throws Exception {
        ...
        String designatedWorker =
                workersList.get(rand.nextInt(workersList.size())).getPath().replaceFirst("/workers/", "");
    ...
    public static void main(String[] args) {
        try {
            CuratorMasterSelector master =
                    new CuratorMasterSelector(args[0], args[1], new ExponentialBackoffRetry(1000, 5));
            master.startZK();
            master.bootstrap();
            master.runForMaster();
        }
